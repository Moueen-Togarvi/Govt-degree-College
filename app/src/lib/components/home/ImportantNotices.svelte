...
<section class="relative overflow-hidden bg-neutral-soft py-24">
	<div class="absolute inset-0 bg-[radial-gradient(circle_at_top,_rgba(247,148,29,0.08),_transparent_36%)]"></div>
			...
			<div class="rounded-[2rem] border border-primary/12 bg-[linear-gradient(135deg,_rgba(255,255,255,0.96),_rgba(247,148,29,0.14))] p-6 lg:p-10">
				<div class="mb-8 rounded-[1.75rem] border border-primary/10 bg-primary px-5 py-5 text-white shadow-[0_18px_50px_rgba(13,93,86,0.20)]">
					<div class="flex flex-wrap items-center justify-between gap-4">
						...
						<div class="rounded-full border border-white/10 bg-white/8 px-4 py-2 text-sm font-black uppercase tracking-[0.22em] text-white/85">
							Pinned Updates
						</div>
					</div>
				...
				<div class="grid gap-6 lg:grid-cols-3">
					{#each boardNotices as notice, index}
						<div
							class={`relative min-h-[280px] rounded-[1.75rem] border border-primary/10 bg-white p-6 shadow-[0_18px_45px_rgba(13,93,86,0.10)] transition-transform duration-300 hover:-translate-y-1 motion-rise ${
								index === 1 ? 'lg:rotate-[-1.5deg]' : index === 2 ? 'lg:rotate-[1.5deg]' : 'lg:-rotate-[1deg]'
							}`}
							style={`animation-delay: ${index * 140}ms`}
						>
							<div class="absolute left-1/2 top-0 h-5 w-5 -translate-x-1/2 -translate-y-1/2 rounded-full border-4 border-white bg-secondary shadow-lg shadow-secondary/30"></div>

							<div class="flex h-full flex-col">
								<div class="mb-5 flex items-start justify-between gap-4">
									<span class="rounded-full border border-secondary/25 bg-secondary/10 px-3 py-1 text-[11px] font-black uppercase tracking-[0.24em] text-secondary">
										{notice.tag}
									</span>
								...
								<h4 class="text-2xl font-black leading-tight text-primary">{notice.title}</h4>
								<p class="mt-4 flex-1 text-sm font-medium leading-7 text-primary/68">{notice.message}</p>

								<div class="mt-5 border-t border-dashed border-primary/10 pt-4">
									<p class="text-xs font-black uppercase tracking-[0.24em] text-primary/45">College Notice Board</p>
								</div>
							</div>
						</div>
					{/each}
