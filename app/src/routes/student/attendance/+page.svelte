		...
		<div>
			...
			<p class="page-subtitle">
				Overall attendance: <strong
					class={overallPct >= 75 ? 'text-green' : overallPct >= 60 ? 'text-yellow' : 'text-red'}
					>{overallPct}%</strong
				>
			</p>
		</div>
		...
		{#each data.attendance as att}
			<div class="summary-card">
				<div class="summary-header">
					...
					<span
						class="pct-badge {att.percentage >= 75
							? 'pct-good'
							: att.percentage >= 60
								? 'pct-warn'
								: 'pct-bad'}">{att.percentage}%</span
					>
				</div>
					...
					<tbody>
						{#each data.detail as entry}
							<tr>
								...
								<td
									><span class="status-pill {statusConfig[entry.status]?.class}"
										>{statusConfig[entry.status]?.label ?? entry.status}</span
									></td
								>
							</tr>
						{/each}
