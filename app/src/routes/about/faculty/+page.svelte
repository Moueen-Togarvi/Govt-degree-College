<script lang="ts">
	...
	): FacultyDirectoryMember | null {
		return department.staff.find((member) => member.isHod) ?? null;
	}

	function getDepartmentCoordinator(
		department: FacultyDirectoryDepartment
	): FacultyDirectoryMember | null {
		return (
			department.coordinator ?? department.staff.find((member) => member.isCoordinator) ?? null
		);
	}
	...
	function getDepartmentFaculty(department: FacultyDirectoryDepartment) {
		...
		return department.staff.filter(
			(member) => member.id !== head?.id && member.id !== coordinator?.id && member.isTeachingStaff
		);
	}
...
<section class="bg-white py-24">
	<div class="container mx-auto px-4 lg:px-8">
		<div class="space-y-14">
			<div
				class="overflow-hidden rounded-[2.25rem] border border-primary/10 bg-[linear-gradient(145deg,rgba(248,249,250,0.96),rgba(255,255,255,1))] p-6 shadow-[0_24px_80px_rgba(13,93,86,0.08)] lg:p-8"
			>
				<div class="flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
					<div>
						<p class="text-sm font-black tracking-[0.32em] text-secondary uppercase">
							Faculty Search
						</p>
						...
						<p class="mt-4 max-w-2xl text-base leading-8 text-primary/65">
							Choose a department, switch language, and view department leadership and faculty in a
					...
					<div class="inline-flex rounded-full border border-primary/10 bg-white p-1.5 shadow-sm">
						<button
							type="button"
							class={`rounded-full px-5 py-3 text-sm font-black tracking-[0.18em] uppercase transition ${
			...
			{#if visibleDepartments.length === 0}
				<div
					class="rounded-[2.5rem] border border-dashed border-primary/20 bg-neutral-soft/70 p-10 text-center text-primary/55"
				>
					<p class="text-sm font-black tracking-[0.3em] text-secondary uppercase">
			...
			{#each visibleDepartments as dept}
				...
				<div
					class="overflow-hidden rounded-[2.5rem] border border-primary/10 bg-[linear-gradient(150deg,rgba(255,255,255,1),rgba(248,249,250,0.94))] p-6 shadow-[0_28px_90px_rgba(13,93,86,0.08)] lg:p-8"
				>
						...
						<div>
							...
							<h2 class="mt-3 text-3xl font-black text-primary lg:text-4xl">
								{activeLanguage === 'english'
									...
									: `${dept.urduName} ڈیپارٹمنٹ`}
							</h2>
						</div>
						<div
							class="inline-flex w-fit rounded-full border border-primary/10 bg-white/80 px-5 py-3 text-sm font-black tracking-[0.22em] text-primary/65 uppercase shadow-sm"
						>
					...
					<div class="mt-8 grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
						<div class="grid gap-6 md:grid-cols-2">
							<div
								class="relative overflow-hidden rounded-[2.25rem] border border-secondary/20 bg-[linear-gradient(145deg,rgba(247,148,29,0.10),rgba(255,255,255,1))] p-6 shadow-[0_24px_70px_rgba(247,148,29,0.10)]"
							>
								<div
									class="absolute top-0 right-0 h-28 w-28 rounded-full bg-secondary/10 blur-3xl"
								></div>
								<div class="relative space-y-5">
									<div
										class="inline-flex rounded-full border border-secondary/20 bg-white/85 px-4 py-2 text-[11px] font-black tracking-[0.28em] text-secondary uppercase shadow-sm"
									>
										{activeLanguage === 'english' ? 'Head of Department' : 'شعبہ سربراہ'}
									</div>
									<div
										class="relative overflow-hidden rounded-[1.8rem] border border-white/80 bg-[linear-gradient(180deg,rgba(233,245,255,0.95),rgba(255,255,255,1))] shadow-lg"
									>
										{#if head}
											<img
												src={head.photo}
											...
											/>
											<div
												class="absolute right-2 bottom-2 h-8 w-8 rounded-full bg-white/40 blur-md"
											></div>
										{:else}
											<div
												class="flex h-72 items-center justify-center bg-neutral-soft text-center text-sm font-bold text-primary/45"
											>
												{activeLanguage === 'english'
													? 'HOD will be added soon'
													: 'HOD جلد شامل کیا جائے گا'}
											</div>
										{/if}
									</div>
									<div class="space-y-2">
										...
										<p class="text-sm font-black tracking-[0.2em] text-primary/45 uppercase">
											{activeLanguage === 'english' ? 'Department Leadership' : 'شعبہ قیادت'}
										</p>
										<p class="text-base leading-7 font-semibold text-primary/70">
											{head
												? activeLanguage === 'english'
													? head.qualification
							...
							<div
								class="relative overflow-hidden rounded-[2.25rem] border border-primary/10 bg-[linear-gradient(145deg,rgba(13,93,86,0.06),rgba(255,255,255,1))] p-6 shadow-[0_24px_70px_rgba(13,93,86,0.10)]"
							>
								<div
									class="absolute bottom-0 left-0 h-28 w-28 rounded-full bg-primary/10 blur-3xl"
								></div>
								<div class="relative space-y-5">
									<div
										class="inline-flex rounded-full border border-primary/10 bg-white/85 px-4 py-2 text-[11px] font-black tracking-[0.28em] text-secondary uppercase shadow-sm"
									>
										{activeLanguage === 'english'
											? 'Department Coordinator'
											: 'ڈیپارٹمنٹ کوآرڈینیٹر'}
									</div>
									<div
										class="relative overflow-hidden rounded-[1.8rem] border border-white/80 bg-[linear-gradient(180deg,rgba(233,245,255,0.95),rgba(255,255,255,1))] shadow-lg"
									>
						...
						<div
							class="rounded-[2.25rem] border border-primary/10 bg-primary px-6 py-7 text-white shadow-[0_24px_70px_rgba(13,93,86,0.20)]"
						>
							<p class="text-sm font-black tracking-[0.3em] text-secondary uppercase">
							...
							<div class="mt-8 grid grid-cols-2 gap-4">
								<div class="rounded-[1.4rem] border border-white/10 bg-white/8 p-4">
									<p class="text-[11px] font-black tracking-[0.24em] text-secondary uppercase">
					...
					<div class="mt-10 flex items-center justify-between gap-4">
						...
						<div
							class="hidden h-px flex-1 bg-gradient-to-r from-secondary/50 to-transparent lg:block"
						></div>
					</div>
					...
					<div class="mt-8 grid grid-cols-2 gap-5 md:grid-cols-3 xl:grid-cols-4">
						{#each getDepartmentFaculty(dept) as member}
							<div
								class="group relative overflow-hidden rounded-[2rem] border border-primary/10 bg-white shadow-[0_18px_45px_rgba(13,93,86,0.08)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_28px_70px_rgba(13,93,86,0.14)]"
							>
								<div
									class="relative overflow-hidden rounded-b-none bg-[linear-gradient(180deg,rgba(233,245,255,0.95),rgba(255,255,255,1))]"
								>
									<img
										src={member.photo}
									...
									<div
										class="absolute right-2 bottom-2 h-6 w-6 rounded-full bg-white/40 opacity-50 blur-md"
									></div>
								</div>
								<div class="bg-[#0D5D56] p-5">
									<div
										class="inline-flex rounded-full border border-secondary/20 bg-secondary/20 px-3 py-1 text-[10px] font-black tracking-[0.24em] text-secondary uppercase backdrop-blur-md"
									>
									...
									<p class="mt-1 line-clamp-1 text-sm leading-5 font-semibold text-white/70">
										{activeLanguage === 'english' ? member.qualification : member.urduQualification}
									</p>
								</div>
							</div>
						{/each}
						...
						{#if getDepartmentFaculty(dept).length === 0}
							<div
								class="rounded-[2rem] border border-dashed border-primary/20 bg-neutral-soft/70 p-8 text-center text-primary/55 md:col-span-2 xl:col-span-3"
							>
								<p class="text-sm font-black tracking-[0.3em] text-secondary uppercase">
