...
function Hero($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		function renderCharacters(value) {
			return Array.from(value, (character) => character === " " ? "\xA0" : character);
		}
		function queueType(delay) {
		...
		for (let lineIndex = 0, $$length = each_array.length; lineIndex < $$length; lineIndex++) {
			...
			$$renderer.push(`<span class="block overflow-hidden"><span${attr_class(`hero-title-line ${lineIndex === 1 ? "text-secondary" : "text-white"}`, "svelte-tc3uts")}><!--[-->`);
			const each_array_1 = ensure_array_like(renderCharacters(line));
			for (let $$index = 0, $$length = each_array_1.length; $$index < $$length; $$index++) {
				let character = each_array_1[$$index];
				$$renderer.push(`<span data-hero-char="" class="hero-char svelte-tc3uts">${escape_html(character)}</span>`);
			}
...
function GalleryPreview($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
			...
			$$renderer.push(`<div class="absolute inset-0 transition-opacity duration-700"><img${attr("src", item.src)}${attr("alt", item.title)} class="absolute inset-0 h-full w-full object-cover object-center [image-rendering:auto]" fetchpriority="high"/> <div class="absolute inset-0 bg-gradient-to-t from-primary via-primary/18 to-transparent"></div> <div class="absolute inset-x-0 bottom-0 p-8 text-white"><p class="text-xs font-black uppercase tracking-[0.28em] text-secondary">${escape_html(item.tag)}</p> <h3 class="mt-3 text-3xl font-black lg:text-4xl">${escape_html(item.title)}</h3></div></div>`);
		}
